...
import { M3uParser, Parser, StreamInfo } from '.';
...
test.beforeEach(async (t) => {
    ...
    await (t.context as Parser).parseM3u('https://iptv-org.github.io/iptv/countries/np.m3u', false);
});
...
test('Save to M3u File', async (t) => {
    await (t.context as Parser).parseM3u('np.m3u');
    await (t.context as Parser).saveToFile('test.m3u');
    t.is(fs.existsSync('test.m3u'), true);
});

test('Save to JSON File', async (t) => {
    await (t.context as Parser).saveToFile('test');
    t.is(fs.existsSync('test.json'), true);
});

// Filtering
test('Filter failed on invalid keySplitter passed', async (t) => {
    await t.throwsAsync(async () => (t.context as Parser).filterBy('tvg-name', ['news'], true, true, '+'));
    await t.throwsAsync(async () => (t.context as Parser).filterBy('tvg-name', ['news'], false, true, '+'));
});
...
test('Sort failed on invalid keySplitter passed', async (t) => {
    await t.throwsAsync(async () => (t.context as Parser).sortBy('tvg-name', true, true, '+'));
    await t.throwsAsync(async () => (t.context as Parser).sortBy('tvg-name', false, true, '+'));
});
...
test('Sort single key: name in ascending order', (t) => {
    ...
    parser.sortBy('name');
    const afterSortedNames = parser.getStreamsInfo().map((stream) => stream.name);
    // check if sorted
...
test('Sort single key: name in descending order', (t) => {
    ...
    const beforeSortedNames = streams.map((stream) => stream.name).sort(sortInOrder(false));
    t.assert(streams.length > 0);
...
test('Sort nestedKey: tvg-id in ascending order', (t) => {
    ...
    const streams: StreamInfo[] = parser.getStreamsInfo();
    t.assert(streams.length > 0);
    const beforeSortedIds = streams.map((stream) => stream.tvg.id).sort(sortInOrder());
    parser.sortBy('tvg-id', true, true);
    const afterSortedIds = parser.getStreamsInfo().map((stream) => stream.tvg.id);
    // check if sorted
...
test('Sort nestedKey: tvg-id in descending order', (t) => {
    ...
    const streams: StreamInfo[] = parser.getStreamsInfo();
    t.assert(streams.length > 0);
    const beforeSortedIds = streams.map((stream) => stream.tvg.id).sort(sortInOrder(false));
