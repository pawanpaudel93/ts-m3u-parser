...
import { NodeParser, StreamInfo } from './common/types';
import { NodeM3uParser as M3uParser } from './node';
...
test.beforeEach(async (t) => {
    ...
    await (t.context as NodeParser).parseM3u(
...
test('Parse a local m3u file', async (t) => {
    await (t.context as NodeParser).parseM3u('np.m3u');
    t.assert((t.context as NodeParser).getStreamsInfo().length > 0);
});

test('Error on parsing invalid path', async (t) => {
    await t.throwsAsync(async () => {
        await (t.context as NodeParser).parseM3u('invalid.m3u');
    });
});
...
test('Error on parsing url/file with no content', async (t) => {
    ...
    await t.throwsAsync(async () => {
        await (t.context as NodeParser).parseM3u('empty.m3u');
    });
});

test('Remove streams by Extension', async (t) => {
    let streams: StreamInfo[] = (t.context as NodeParser).getStreamsInfo();
    t.assert(streams.length > 0);
    (t.context as NodeParser).removeByExtension(['.m3u8']);
    streams = (t.context as NodeParser).getStreamsInfo();
...
test('Retrieve streams by category', async (t) => {
    let streams: StreamInfo[] = (t.context as NodeParser).getStreamsInfo();
    t.assert(streams.length > 0);
    (t.context as NodeParser).retrieveByCategory(['Music']);
    streams = (t.context as NodeParser).getStreamsInfo();
    t.assert(streams.length > 0);
});
...
test('Reset operations', (t) => {
    (t.context as NodeParser).removeByExtension(['m3u8']);
    t.is((t.context as NodeParser).getStreamsInfo().length, 0);
    (t.context as NodeParser).resetOperations();
    t.assert((t.context as NodeParser).getStreamsInfo().length > 0);
});

test('Save to M3u File', async (t) => {
    await (t.context as NodeParser).parseM3u('np.m3u');
    (t.context as NodeParser).saveToFile('test.m3u');
    t.is(fs.existsSync('test.m3u'), true);
});

test('Save to JSON File', (t) => {
    (t.context as NodeParser).saveToFile('test');
...
test('Filter failed on invalid keySplitter passed', async (t) => {
    await t.throwsAsync(async () =>
        (t.context as NodeParser).filterBy(
            'tvg-name',
            ['news'],
            true,
            true,
...
test('Filter failed on no filters passed', async (t) => {
    const beforeStreams = (t.context as NodeParser).getStreamsInfo();
    (t.context as NodeParser).filterBy('name', []);
    const afterStreams = (t.context as NodeParser).getStreamsInfo();
...
test('Sort failed on invalid keySplitter passed', async (t) => {
    await t.throwsAsync(async () =>
        (t.context as NodeParser).sortBy('tvg-name', true, true, '+')
    );
    await t.throwsAsync(async () =>
        (t.context as NodeParser).sortBy('tvg-name', false, true, '+')
    );
});

test('Sort single key: name in ascending order', (t) => {
    const parser = t.context as NodeParser;
...
test('Get a random stream', async (t) => {
    const stream = (t.context as NodeParser).getRandomStream();
